...
def query_usgs_catalog(
    ...
    # Retry once on transient network failures so a brief blip cannot zero out
    # the historical-events table during a live demo.
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                time.sleep(2.5)  # brief backoff before the retry
                continue
